...
public class RichFormatParser {
	...
	private LinkedList<RichFormat> formats = new LinkedList<>();
		...
		for (int i = 0; i < c.length; i++) {
			...
			if (curChar == '*') {
				...
				if (currentFormat == null) {
					...
					currentString = new StringBuffer();
					currentFormat = null;
				} else if ((currentFormat == Format.BOLD || currentFormat == Format.ITALIC) && subFormat == null) {
					if (f == Format.ITALIC) {
	...
	private int getDoubleFormat(String s, int pos, char followingChar) {
		...
		if (s.charAt(pos) == followingChar) {
			do {
				count++;
			} while ((pos + count) < s.length() && s.charAt(pos + count) == followingChar);
		} else {
			while (((pos + 1) + count) < s.length() && s.charAt((pos + 1) + count) == followingChar) {
				count++;
			}
		}
		return count;
